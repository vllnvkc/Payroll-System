...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['log_attendance'])) {
    ...
    // Fetch the `id`, `email`, and `schedule_id` corresponding to the entered `employee_id`
    $get_employee_query = "SELECT id, email, schedule_id FROM employees WHERE employee_id = '$employee_id_input'";
    ...
    if (mysqli_num_rows($employee_result) > 0) {
        ...
        $schedule_id = $employee_data['schedule_id']; // Get the schedule ID

        // Fetch the employee's afternoon schedule using `schedule_id`
        $schedule_query = "SELECT time_in_afternoon FROM schedules WHERE id = '$schedule_id'";
        $schedule_result = mysqli_query($connection, $schedule_query);

        if (mysqli_num_rows($schedule_result) > 0) {
            $schedule = mysqli_fetch_assoc($schedule_result);
            $start_time = $schedule['time_in_afternoon'];

            // Determine status (0 = Late, 1 = On Time)
            $status_afternoon = (strtotime($current_time) > strtotime($start_time)) ? 0 : 1;
            ...
            if (mysqli_num_rows($attendance_result) > 0) {
                ...
                // Alert if both morning and afternoon time-ins already exist
                if (!is_null($attendance_row['time_in_afternoon'])) {
                    echo "<script>alert('Attendance already exists for this afternoon.');</script>";
                } else {
                    // Update time-in for afternoon only
                    $update_attendance_query = "
                        UPDATE attendance 
                        SET time_in_afternoon = '$current_time', status_afternoon = '$status_afternoon'
                        WHERE employee_id = '$employee_id' AND date = '$current_date'
                    ";
                    mysqli_query($connection, $update_attendance_query);

                    // Send confirmation email
                    sendEmail($employee_email, "Afternoon Time-in", $current_time, $current_date);
                    echo "<script>alert('Afternoon Time-in recorded successfully and confirmation email sent.');</script>";
                }
            } else {
                ...
                $insert_attendance_query = "
                    INSERT INTO attendance (attendance_id, employee_id, date, month, year, time_in_afternoon, status_afternoon) 
